...
public:
  ...
  bool birth(int row, int col, int &id)
  {
    ...
    if (usedNum != nullptr)
    {
      std::cout<<"used num used for new child\n";
      tempID = usedNum->ID;
      Node *temp = usedNum;
  ...
  bool death(int personID)
  {
    ...
    {
      districts.at(dead->row).at(dead->col)->head = nullptr;
    }
    else
    {
  ...
  bool move(int id, int targetRow, int targetCol)
  {
    ...
    {
      std::cout<<"new district head adjusted";
  ...
  std::vector<int> *members(int row, int col) const
  {
    ...
    std::cout<<districts.at(row).at(col)->population;
